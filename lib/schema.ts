 ...
 * the first request.
 *
 * Statements are applied one at a time and are all idempotent, so several
 * instances can migrate concurrently without stepping on each other.
 */
export const SCHEMA_STATEMENTS: string[] = [
  ...
  // One canonical URL per post: the slug is unique inside its primary service.
  `CREATE UNIQUE INDEX IF NOT EXISTS idx_posts_service_slug
     ON posts (primary_service_id, slug) WHERE slug <> ''`,
  ...
  // Images live in the database too. On a serverless host the filesystem is
  // per-instance, so a file written by one request is invisible to the next.
